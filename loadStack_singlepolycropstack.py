...
if __name__ == '__main__':
    ...
    for f in files:
        ...
        if (os.path.isfile(f) and f.endswith(ext)):
            ...
            # print tileIDs
            try:
                polypts = canvasImage.polygon[0].worldCoords
            except IndexError:
                continue
    ...
    tsjsons = []
    for zidx in renderapi.stack.get_z_values_for_stack(inputStack, render=r):
        z_ts = renderapi.tilespec.get_tile_specs_from_z(
            inputStack, z, render=r)
        tilespecs_inside = select_tilespecs_inside_polygon(polypts, z_ts)
